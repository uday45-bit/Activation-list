...
An office is **ACTIVE** if **either** of the following is true:

| Mode | Condition 1 (count) | Condition 2 (amount) |
|---|---|---|
| QR   | QR Count ≥ 10   | QR Amount ≥ ₹5,000   |
| Card | Card Count ≥ 10 | Card Amount ≥ ₹5,000 |

**Overall = ACTIVE if (QR meets both conditions) OR (Card meets both conditions).**
...
- The activation rule (OR between QR/Card) is defined once as constants at
  the top of `app.py` — change `MIN_TRANSACTIONS` / `MIN_AMOUNT` there if the
  policy changes.
