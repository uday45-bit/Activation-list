...
The app shows a single **color-coded division-wise summary table**:

- Heading shows the report period as `dd.mm.yyyy to dd.mm.yyyy` — from the
  1st of the current month to the date the report is generated (today).
- Columns: Division, Total Offices, Active, Inactive, Activation %, with a
  bolded **All Divisions** grand-total row at the bottom.
- Row color: **green** if that division's activation % is **≥ 50%**, **red**
  if **< 50%**.
- Column widths are fixed/proportional so the table stays tidy regardless of
  division name length.

Full office-level detail and any unmapped offices are tucked under a
collapsed **"Full office-level detail / unmapped offices"** expander below
the table, and the downloadable Excel report mirrors the same color-coded
division summary (plus a full-detail sheet) for offline use.
