...
with st.expander("ℹ️ Activation rule used in this app", expanded=False):
    st.markdown(
        f"""
An office's POS machine is marked **ACTIVE** if **either** of the two payment
modes independently satisfies both its conditions:

| Mode | Condition 1 (transactions) | Condition 2 (amount) |
|---|---|---|
| **QR**  | QR Count ≥ {MIN_TRANSACTIONS} | QR Amount ≥ ₹{MIN_AMOUNT:,} |
| **Card**| Card Count ≥ {MIN_TRANSACTIONS} | Card Amount ≥ ₹{MIN_AMOUNT:,} |

**Overall status = ACTIVE if (QR meets both conditions) OR (Card meets both conditions).**
...
if master_file and txn_file:
    ...
    if st.button("🚀 Compute Activation Status", type="primary"):
        ...
        merged["QR Active"] = (merged["QR Count"] >= MIN_TRANSACTIONS) & (
            merged["QR Amount"] >= MIN_AMOUNT
        )
        merged["Card Active"] = (merged["Card Count"] >= MIN_TRANSACTIONS) & (
            merged["Card Amount"] >= MIN_AMOUNT
        )
        merged["Activation Status"] = np.where(
            merged["QR Active"] | merged["Card Active"], "ACTIVE", "INACTIVE"
        )
        merged["QR Active"] = merged["QR Active"].map({True: "Yes", False: "No"})
        merged["Card Active"] = merged["Card Active"].map({True: "Yes", False: "No"})
