...
def build_pos_supply_summary(master_df, supply_summary):
    """Division-wise supply/whitelisting summary with a Remarks column
    flagging any excess machines supplied vs. the office count in that
    division (per the current filtered office universe)."""
    total_by_div = master_df.groupby("Division Name").size().reset_index(name="Total Offices")
    out = pd.merge(total_by_div, supply_summary, on="Division Name", how="outer")
    out["Total Offices"] = out["Total Offices"].fillna(0).astype(int)
    for c in ["Machines Supplied", "Not Whitelisted Count", "Not Supplied Count"]:
        out[c] = out[c].fillna(0).astype(int)

    def remark(r):
        excess = r["Machines Supplied"] - r["Total Offices"]
        return f"Excess of {excess} machine(s) supplied vs. office count" if excess > 0 else ""

    out["Remarks"] = out.apply(remark, axis=1)
    return out.sort_values("Division Name").reset_index(drop=True)
...
def build_missing_offices_summary(merged):
    """Offices present in master data but absent from today's transaction
    file entirely (distinct from 'inactive', which can include offices that
    reported some transactions but stayed below threshold)."""
    missing_div = (
        merged.groupby("Division Name")["In Transaction File"]
        .apply(lambda s: (~s).sum())
        .reset_index(name="Missing Count")
    )
    missing_div["Total Offices"] = merged.groupby("Division Name")["Office Name"].count().values
    missing_div["% Missing"] = round(missing_div["Missing Count"] / missing_div["Total Offices"] * 100, 1)
    missing_div = missing_div.sort_values("% Missing", ascending=False).reset_index(drop=True)

    missing_detail = (
        merged[~merged["In Transaction File"]][["Division Name", "Office ID", "Office Name", "Office Type"]]
        .sort_values(["Division Name", "Office Name"])
        .reset_index(drop=True)
    )
    return missing_div, missing_detail
...
def build_excel_report(merged, div_summary, missing_div, missing_detail, supply_summary, whitelist_detail, notsupplied_detail, period_str):
    output = BytesIO()
    with pd.ExcelWriter(output, engine="xlsxwriter") as writer:
        ...
        # ---- Sheet 3: Missing From Feed (division-wise) ----
        ws3 = workbook.add_worksheet("Missing From Feed")
        ws3.merge_range(0, 0, 0, 3, f"Offices Missing from Transaction File — Division-wise (Period: {period_str})", title_fmt)
        headers3 = ["Division", "Total Offices", "Missing Count", "% Missing"]
        for col, h in enumerate(headers3):
            ws3.write(1, col, h, header_fmt)
        row_idx = 2
        for _, row in missing_div.iterrows():
            fmt, fmt_left = (red_fmt, red_fmt_left) if row["% Missing"] >= 25 else (plain_fmt, plain_fmt_left)
            ws3.write(row_idx, 0, row["Division Name"], fmt_left)
            ws3.write(row_idx, 1, int(row["Total Offices"]), fmt)
            ws3.write(row_idx, 2, int(row["Missing Count"]), fmt)
            ws3.write(row_idx, 3, row["% Missing"], fmt)
            row_idx += 1
        total_missing = int(missing_div["Missing Count"].sum())
        total_missing_pct = round(total_missing / total_offices * 100, 1) if total_offices else 0
        ws3.write(row_idx, 0, "Total", total_fmt_left)
        ws3.write(row_idx, 1, total_offices, total_fmt)
        ws3.write(row_idx, 2, total_missing, total_fmt)
        ws3.write(row_idx, 3, total_missing_pct, total_fmt)
        ws3.set_column(0, 0, 32)
        ws3.set_column(1, 3, 16)

        # ---- Sheet 4: Missing Offices detail ----
        missing_detail.to_excel(writer, sheet_name="Missing Offices Detail", index=False)
        ws4 = writer.sheets["Missing Offices Detail"]
        ws4.set_column(0, 0, 30)
        ws4.set_column(1, 1, 14)
        ws4.set_column(2, 2, 32)
        ws4.set_column(3, 3, 14)

        # ---- Sheet 5: POS Supply & Whitelisting (division-wise, with excess Remarks) ----
        ws5 = workbook.add_worksheet("POS Supply & Whitelisting")
        ws5.merge_range(0, 0, 0, 5, "SBI POS Machines — Supply & Whitelisting Status (Division-wise)", title_fmt)
        headers5 = ["Division", "Total Offices", "Machines Supplied", "Not Whitelisted", "Not Supplied", "Remarks"]
        for col, h in enumerate(headers5):
            ws5.write(1, col, h, header_fmt)
        row_idx = 2
        for _, row in supply_summary.iterrows():
            has_issue = bool(row["Remarks"])
            fmt, fmt_left = (red_fmt, red_fmt_left) if has_issue else (plain_fmt, plain_fmt_left)
            ws5.write(row_idx, 0, row["Division Name"], fmt_left)
            ws5.write(row_idx, 1, int(row["Total Offices"]), fmt)
            ws5.write(row_idx, 2, int(row["Machines Supplied"]), fmt)
            ws5.write(row_idx, 3, int(row["Not Whitelisted Count"]), fmt)
            ws5.write(row_idx, 4, int(row["Not Supplied Count"]), fmt)
            ws5.write(row_idx, 5, row["Remarks"], fmt_left)
            row_idx += 1
        ws5.write(row_idx, 0, "Total", total_fmt_left)
        ws5.write(row_idx, 1, int(supply_summary["Total Offices"].sum()), total_fmt)
        ws5.write(row_idx, 2, int(supply_summary["Machines Supplied"].sum()), total_fmt)
        ws5.write(row_idx, 3, int(supply_summary["Not Whitelisted Count"].sum()), total_fmt)
        ws5.write(row_idx, 4, int(supply_summary["Not Supplied Count"].sum()), total_fmt)
        ws5.write(row_idx, 5, "", total_fmt_left)
        ws5.set_column(0, 0, 30)
        ws5.set_column(1, 4, 16)
        ws5.set_column(5, 5, 40)

        # ---- Sheet 6: Whitelisting Pending (detail) ----
        whitelist_detail.to_excel(writer, sheet_name="Whitelisting Pending", index=False)
        ws6 = writer.sheets["Whitelisting Pending"]
        ws6.set_column(0, 0, 30)
        ws6.set_column(1, 1, 14)
        ws6.set_column(2, 2, 32)

        # ---- Sheet 7: Not Supplied Offices (detail) ----
        notsupplied_detail.to_excel(writer, sheet_name="Not Supplied Offices", index=False)
        ws7 = writer.sheets["Not Supplied Offices"]
        ws7.set_column(0, 0, 30)
        ws7.set_column(1, 1, 32)
...
with st.container(border=True):
    ...
    override_upload = st.file_uploader(
        "Replace master office file (this session only). For a permanent monthly update, "
        "replace master_data.csv in the GitHub repo and push.",
    ...
    supply_upload = st.file_uploader(
        "Replace POS supply & whitelisting file (this session only). For a permanent update, "
        f"replace {POS_SUPPLY_FILENAME} in the GitHub repo and push.",
        type=["xlsx", "xls"], key="supply_upload",
    )
...
if txn_file:
    ...
    merged["In Transaction File"] = merged["Merge Key"].isin(set(txn["Merge Key"]))
    for c in ["QR Count", "QR Amount", "Card Count", "Card Amount"]:
    ...
    missing_div, missing_detail = build_missing_offices_summary(merged)
    supply_summary = build_pos_supply_summary(master_df, supply_summary_raw)

    image_bytes = build_division_summary_image(div_summary, period_str)
    excel_bytes = build_excel_report(
        merged, div_summary, missing_div, missing_detail,
        supply_summary, whitelist_detail, notsupplied_detail, period_str,
    )
